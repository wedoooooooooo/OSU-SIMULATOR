...
while True:
    ...
    for event in pygame.event.get():
        ...
        if pygame.sprite.collide_rect(player_real, ball_real):
            ...
            ## (line 84, 85) if the player's mouse pos is colliding with the ball and the game receives an X or Z input, then the following happens:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_x or event.key == pygame.K_z:
                    ...
                    # line 98 prints the new location of the ball, just to ensure that the janky code works xd (just uncomment to use)
                    # print("(" + str(x) + ", " + str(y) + ")")
